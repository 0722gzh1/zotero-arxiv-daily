...
executor:
  ...
  min_theme_score: 7.0 # Minimum LLM theme score required to send a paper.
...
## 📖 How it works
*Zotero-arXiv-Daily* firstly retrieves all the papers in your Zotero library and all the papers released in the previous day, via corresponding API. Then it calculates the embedding of each paper's title and abstract via an embedding model. The score of a paper is based mainly on the most similar Zotero papers and a light recency preference for recently added Zotero papers. An LLM theme judge then filters out papers that only share generic method words but are not in the same research theme. The email shows the closest Zotero matches, theme-match score, and the judge reason as recommendation evidence. The TLDR of each paper and the daily research briefing are generated by LLM.
