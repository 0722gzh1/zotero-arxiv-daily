...
reranker:
  relevance:
    top_k: 20 # Score each candidate by its closest Zotero papers instead of averaging over the entire library.
    match_count: 3 # Number of closest Zotero papers shown as recommendation evidence in the email.
    best_similarity_weight: 0.3 # Blend in the single best Zotero match. Must be between 0 and 1.
...
## 📖 How it works
*Zotero-arXiv-Daily* firstly retrieves all the papers in your Zotero library and all the papers released in the previous day, via corresponding API. Then it calculates the embedding of each paper's title and abstract via an embedding model. The score of a paper is based mainly on the most similar Zotero papers, blended with the single strongest Zotero match and a light recency preference for recently added Zotero papers. The email shows the closest Zotero matches as recommendation evidence. The TLDR of each paper and the daily research briefing are generated by LLM.
