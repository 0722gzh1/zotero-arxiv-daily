...
topic_profiles:
  enabled: true # Generate temporary research-interest profiles from your Zotero library on each workflow run.
  auto_generate: true
  min_topics: 2
  max_topics: 8
  max_papers_for_profile: 80
  match_top_n: 3
...
source:
  ...
  biorxiv:
    category: null # Ignored. bioRxiv retrieves all papers from the latest date returned by its 2-day API window.
  medrxiv:
    category: null # Ignored. medRxiv retrieves all papers from the latest date returned by its 2-day API window.

topic_profiles:
  enabled: false # Whether to judge papers against explicit research-interest topic profiles.
  auto_generate: true # Generate temporary profiles from the Zotero corpus on each workflow run.
  min_topics: 2 # Lower bound for automatically inferred topic count.
  max_topics: 8 # Upper bound for automatically inferred topic count.
  max_papers_for_profile: 80 # Zotero papers summarized into the temporary profile prompt.
  path: config/topic_profiles.yaml # Optional YAML file used when auto_generate is false and topics is empty.
  refresh_days: 30 # Reserved for future automatic topic refresh.
  match_top_n: 3 # Number of candidate topic profiles attached to each paper before LLM judging.
  topics: [] # Optional inline topic profile list. If empty, topic_profiles.path is used when enabled.

theme_judge:
  min_core_score: 7.5 # Minimum LLM theme score for core recommendations.
  min_peripheral_score: 6.5 # Minimum LLM theme score for peripheral-but-useful recommendations.
...
reranker:
  relevance:
    top_k: 10 # Score each candidate by its closest Zotero papers instead of averaging over the entire library.
    match_count: 3 # Number of closest Zotero papers shown as recommendation evidence in the email.
    best_similarity_weight: 0.0 # Blend in the single best Zotero match. Must be between 0 and 1.
    time_decay_strength: 0.15 # Lightly prefer Zotero papers added recently. Set 0 to disable.
  local:
...
executor:
  ...
  min_relevance_score: null # Minimum relevance score required to send a paper. Example: 6.0
  theme_judge: true # Whether to use LLM to filter papers by true research-theme match.
  theme_judge_paper_num: 30 # Number of top embedding-ranked papers judged by the LLM theme filter.
  min_theme_score: 7.0 # Legacy fallback threshold if theme_judge.min_core_score is not set.
  theme_judge_keep_on_failure: false # Whether to keep a paper if LLM theme judging fails.
  daily_overview: true # Whether to generate a daily research briefing at the top of the email.
  overview_paper_num: 12 # Number of top-ranked papers included in the daily research briefing prompt.
...
## 📖 How it works
*Zotero-arXiv-Daily* firstly retrieves all the papers in your Zotero library and all the papers released in the previous day, via corresponding API. When topic profiles are enabled, it generates temporary research-interest profiles from the current Zotero corpus during the workflow run, using the existing `ZOTERO_ID` and `ZOTERO_KEY` environment variables. The number of profiles is inferred from library size and collection diversity. Then it calculates embedding scores between candidate papers, Zotero papers, and topic profiles. An LLM theme judge filters out papers that only share generic method words but are not in the same research theme. The email shows the matched topic, closest Zotero matches, theme-match score, lane, detailed evidence, and judge reason. The TLDR of each paper and the daily research briefing are generated by LLM.
